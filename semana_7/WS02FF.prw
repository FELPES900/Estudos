...
WSMETHOD POST WSSERVICE POSTCLI
	...
	if Empty(cError)
		...
		for nX := 1 to Len(oJson:GetJsonObject('CLIENTES'))
			...
			if !SA1->(MsSeek( PadR(oJson:GetJsonObject( 'CLIENTES' )[nX][ 'FILIAL' ],TamSx3('A1_FILIAL')[1]) + PadL(oJson:GetJsonObject( 'CLIENTES' )[nX][ 'ID' ],TamSx3('A1_COD')[1],'0')   +  cLoja))
				aClient := {}
				aadd(aClient, {"A1_FILIAL" , oJson:GetJsonObject( 'CLIENTES' )[nX][ 'FILIAL' ]                                                           , NIL})
